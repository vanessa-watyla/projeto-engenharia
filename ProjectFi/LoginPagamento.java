...
public class LoginPagamento {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					LoginPagamento window = new LoginPagamento();
	...
	private void initialize() {
		...
		frmLoginPag.setTitle("ZEUZ ACADEMIA");
...
//Botão de acessar o menu.
		...
		Logar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!Usuario.getText().isEmpty() && !Senha.getText().isEmpty()) {
					...
					if( Confirmar==0) {JOptionPane.showMessageDialog(null, "Usuario ou senha incorreto.");
					Senha.setText("");
					Usuario.setText("");
					}
				} else {
		...
		lblNewLabel.setIcon(new ImageIcon("C:\\Users\\Gabriel Sacramento\\Downloads\\icons8-enter-50.png"));
		lblNewLabel.setBounds(28, 24, 39, 38);
		frmLoginPag.getContentPane().add(lblNewLabel);

		JLabel lblNewLabel_1 = new JLabel("Login para Pagamento");
		lblNewLabel_1.setFont(new Font("Avignon Pro Xlight", Font.BOLD, 14));
		lblNewLabel_1.setBounds(120, 10, 190, 13);
		...
		lblNewLabel_2.setBounds(0, 175, 245, 13);
